import { PartyPersona } from "./parties";
 ...
 */
export function buildDirectQuestionPrompt(
  party: PartyPersona,
  ...
  const contextText = context
    .map(
      (c, i) =>
        `[Källa ${i + 1} – Avsnitt: "${c.sourceSection}", Sida ${c.pageNumber}]\n${c.content}`
    )
    .join("\n\n---\n\n");
...
${hasContext ? `TILLGÄNGLIG MANIFEST-KONTEXT:\n${contextText}` : "MANIFEST-KONTEXT: Basera svaret på partiets allmänna ideologi och politiska linje."}
...
${hasContext ? `MANIFEST-KONTEXT:\n${contextText}` : "MANIFEST-KONTEXT: Argumentera utifrån partiets kända ideologi och politiska prioriteringar."}
